...
/**
 * This class contains Lab3 Navigation Lab implemented on the EV3 platform. This class specifies
 * constants that control the behaviour of the robot. This class also creates instances of the left
 * and right motors (two Large EV3 motors), and also the LCD display, and the medium EV3 motor that
 * turns the ultrasonic sensor and the information to be displayed. Users are given the option of
 * choosing a mode of operation, whether to only navigate or navigate with obstacle avoidance
 * feature. The Odometer, OdometerDisplay, navigation, and sensor motor threads are started based on
 * the button clicked by the user. The robot drives to 5 waypoints defined by the user after program
 * starts. The program exits upon clicking any button after starting the program and choosing a mode
 * of operation.
 * 
 * @author Raymond Yang
...
public class Lab3 {
  ...
  /**
   * The radius (in cm) of the left/right wheels of the EV3 robot. This constant in THIS class is
   * only used for Odometer purposes
   */
...
//      { 0, 1 }, { 1, 2 }, { 1, 0 }, { 2, 1 }, { 2, 2 }
  ...
  /**
   * The port that the ultrasonic sensor is connected to, which is port S1 on the EV3 brick.
   */
  ...
  /**
   * The main method for Lab3 class. In this class, the user options are specified and formatted.
   * Responses to button clicks are specified. The odometer, LCD display, ultrasonic sensor,
   * navigator (simple or with obstacle avoidance), and ultrasonic sensor motor are created. The
   * corresponding threads are started, allowing simultaneous execution of multiple tasks (rotating
   * sensor, going forward, etc.)
   * 
  ...
  public static void main(String[] args) throws OdometerExceptions {
    ...
    } else if (buttonChoice == Button.ID_RIGHT) { // RIGHT button chosen: navigation with obstacle
                                                  // avoidance

      @SuppressWarnings("resource") // Because we don't bother to close this resource
      SensorModes usSensor = new EV3UltrasonicSensor(US_PORT); // provide access to selecting a mode
      SampleProvider usDistance = usSensor.getMode("Distance"); // provides access to acquire sensor
                                                                // data
