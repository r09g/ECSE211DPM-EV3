...
/**
 * <p>
 * This class implements the advanced navigator with the ultrasonic sensor. Some of the existing
 * motor instances and a few constants not dependent on navigation. This class extends the Thread
 ...
 * 
 * <p>
 * The robot uses the ultrasonic sensor to detect obstacles and avoid them if necessary, using
 * either the left bangbang mode or the right bangbang mode. There are many constants in this class
 * that are related to the bangbang controllers.
 * 
 * @author Raymond Yang
...
public class USNav extends Thread {
  ...
  /**
   * The radius (in cm) of the left/right wheels of the EV3 robot. This constant is not imported
   * from Lab3 class specifically for tuning the obstacle avoidance navigator. TWEAKING THIS VALUE
   * SIGNIFICANTLY CHANGES HOW MUCH OF AN ANGLE THE ROBOT TURNS.
   */
  public static final double WHEEL_RAD = 2.1;

  /**
   * The width (in cm) of the robot measured from the center of the left wheel to the center of the
   * right wheel. Not imported from Lab3 because track needs to be tuned specifically for obstacle
  ...
  /**
   * Avoid hunting behaviour. Within this range (in cm) from the band center, the bangbang
   * controller does not make adjustments
   */
  ...
  /**
   * The distance that triggers the bangbang controller to avoid the obstacle (in cm)
   */
  private static final int AVOID_DISTANCE = 15;

  /**
   * A value that controls the maximum distance (in cm) the robot travels in bangbang mode, allows
   * exiting of obstacle avoidance state
   */
  private static final int MAX_AVOID_DISTANCE = 55;

  /**
   * A signal indicating the robot is currently not in obstacle avoidance mode. This is mainly used
   * to signal the ultrasonic motor to sweep the sensor
   */
  private static final int NOT_AVOIDING = 0;

  /**
   * A signal indicating the robot is currently in right bangbang mode. This is mainly used to
   * signal the ultrasonic motor to turn the motor left to face the wall and hold it there
   */
  private static final int RIGHT_BANGBANG = 1;

  /**
   * A signal indicating the robot is currently in left bangbang mode. This is mainly used to signal
   * the ultrasonic motor to turn the motor right to face the wall and hold it there
   */
  private static final int LEFT_BANGBANG = 2;

  /**
   * The clockwise angle turned from Theta = 0 (north). This is the 45 degree line in quadrant 2.
   * This is used in deciding whether to execute left bangbang or right bangbang
   */
  private static final int TOP_LEFT = 315;

  /**
   * The clockwise angle turned from Theta = 0 (north). This is the 45 degree line in quadrant 1.
   * This is used in deciding whether to execute left bangbang or right bangbang
   */
  private static final int TOP_RIGHT = 45;

  /**
   * The clockwise angle turned from Theta = 0 (north). This is the 45 degree line in quadrant 3.
   * This is used in deciding whether to execute left bangbang or right bangbang
   */
  private static final int BOTTOM_LEFT = 225;

  /**
   * The clockwise angle turned from Theta = 0 (north). This is the 45 degree line in quadrant 4.
   * This is used in deciding whether to execute left bangbang or right bangbang
   */
  private static final int BOTTOM_RIGHT = 135;

  /**
   * This is the threshold for ultrasonic sensor readings. Values above this threshold activate the
   * tally (counter) and tells the robot that it might need to make a turn rather than adjustments
   */
  private static final int DISTANCE_THRESHOLD = 160;

  /**
   * The threshold needed for the tally (counter). If this is satisfied then a left turn is executed
   * rather than left adjustments
   */
  ...
  /**
   * The motor instance that controls the sweeping of the ultrasonic sensor. Motor used is the EV3
   * medium motor
   */
  ...
  /**
   * Stores the distance reading, after applying a median medianFilter, from the ultrasonic sensor
   */
  private double distance;

  /**
   * Records the distance travelled (in cm) after the occurrence of an obstacle in the robot's path.
   * This is used to control the length of the bangbang controller execution and assist in the
   * exiting of the obstacle avoidance state
   */
  private double turncount;

  /**
   * This is a boolean flag which signals whether the robot has reached the destination coordinate.
   * Useful for navigation after avoiding an obstacle
   */
  ...
  /**
   * This constructor passes in the existing instances of the odometer and the ultrasonic sensor
   * related instances. The values of turncount and navigating status are also initialized here to
   * 0. Note that motor related instances are imported as static objects at the top of this class.
   *
   * @param odometer - the existing odometer instance, allows acquiring position data
   * @param us - access to acquiring ultrasonic sensor reading datas
   * @param usData - buffer to store the ultrasonic sensor readings
   * @param usMotor - access to controlling and checking states of the motor that controls the
   *        sweeping of sensor
   */
   ...
   * There is a completed flag in this method to account for the case when the robot avoided an
   * obstacle. The robot is not at the destination but the travelTo() method returned. Robot needs
   * to re-navigate and go to the same destination coordinate again.
   */
  public void run() {
   ...
   * 
   * <p>
   * The robot will check for ultrasonic sensor readings when the left and right motors are moving,
   * to identify any obstacles and avoid them using the bangbang controller. The robot stops when
   * encountering an obstacle avoidance and follows bangbang controller procedure (robot avoids from
   * either left side or right side). After obstacle avoidance ends, the ultrasonic sensor motor is
   * signaled to turn to a specific angle to allow bangbang; the completed flag is set to signal
   * re-navigation; and the turn count is reset to 0. If no obstacle is encountered, the robot
   * behaves the same as in simple navigation
   * 
  ...
  /**
   * Getter method to determine whether robot is currently navigating by checking class variable
   * {@code isNavigating}
   * 
  ...
  /**
   * This method consists of a set of if-else statements and is used to determine whether to avoid
   * the obstacle from the left side or from the right side
   */
  ...
  /**
   * Original version of bangbang controller that avoids the obstacle from the right side. The robot
   * is turned 90 degrees to the right and the ultrasonic sensor is turned to face left before
   * starting Rbangbang() mode
   */
  ...
  /**
   * Mirror version of bangbang controller that avoids the obstacle from the left side. The robot is
   * turned 90 degrees to the left and the ultrasonic sensor is turned to face right before starting
   * Lbangbang() mode
   */
