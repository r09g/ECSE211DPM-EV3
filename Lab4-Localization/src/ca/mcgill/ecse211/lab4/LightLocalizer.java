...
public class LightLocalizer {
  ...
  double thetaY, thetaX, x, y, dTheta, ThetaNegY, tx1, tx2, ty1, ty2; //TODO: may want to add ThetaNegX
  double intensity;

  public LightLocalizer(int buttonChoice, SampleProvider lightColor, float[] lightData)
      ...
      if (worked) {
        ...
        while (true) {
          ...
          if ((intensity / std) < THRESHOLD) { //detected a horizontal gridline
            LEFT_MOTOR.stop();
            RIGHT_MOTOR.stop();
            LEFT_MOTOR.rotate(-convertAngle(WHEEL_RAD, TRACK, 45), true); // turns left, so facing north again
            RIGHT_MOTOR.rotate(convertAngle(WHEEL_RAD, TRACK, 45), true);
            LEFT_MOTOR.forward();
            RIGHT_MOTOR.forward();
            LEFT_MOTOR.setSpeed(SPEED);
